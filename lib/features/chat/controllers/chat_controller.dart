...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> searchConversation(String name) async {
    ...
    if (searchConversationModel != null) {
      ...
      //
      // log('searchConversationModel: ${index0}');
      //
      // if(index0 != -1) {
      //   if(sender) {
  ...
  Future<Response?> sendMessage(
      {required String message, required NotificationBodyModel? notificationBody, required int? conversationID, required int? index}) async {
    Response? response;
    _isLoading = true;
    update();
    ...
    if (response!.statusCode == 200) {
      ...
      if (_conversationModel != null &&
          !_hasAdmin &&
          (_messageModel!.conversation!.senderType == UserType.admin.name || _messageModel!.conversation!.receiverType == UserType.admin.name) &&
  ...
  void _sortMessage(int? adminId) {
    if (_messageModel!.conversation != null &&
        (_messageModel!.conversation!.receiverType == UserType.user.name || _messageModel!.conversation!.receiverType == UserType.customer.name)) {
