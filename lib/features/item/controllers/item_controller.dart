...
class ItemController extends GetxController implements GetxService {
  ...
  Future<int> setExistInCart(Item? item, List<List<bool?>>? selectedVariations, {bool notify = false}) async {
    ...
    if (ModuleHelper.getModuleConfig(ModuleHelper.getModule() != null ? ModuleHelper.getModule()!.moduleType : ModuleHelper.getCacheModule()!.moduleType)
        .newVariation!) {
  ...
  void setQuantity(bool isIncrement, int? stock, int? quantityLimit, {bool getxSnackBar = false}) {
    _quantity = itemServiceInterface.setQuantity(
        isIncrement, Get.find<SplashController>().configModel!.moduleConfig!.module!.stock!, stock, _quantity!, quantityLimit,
  ...
  void itemDirectlyAddToCart(Item? item, BuildContext context, {bool inStore = false, bool isCampaign = false}) {
    if (((item!.foodVariations != null && item.foodVariations!.isEmpty) && item.moduleType == AppConstants.food) ||
        (item.variations != null && item.variations!.isEmpty && item.moduleType != AppConstants.food)) {
      ...
      if (Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! && item.stock! <= 0) {
        showCustomSnackBar('out_of_stock'.tr);
          ...
          cartModel.item!.storeId, ModuleHelper.getModule() != null ? ModuleHelper.getModule()?.id : ModuleHelper.getCacheModule()?.id)) {
        Get.dialog(
            ConfirmationDialog(
              icon: Images.warning,
