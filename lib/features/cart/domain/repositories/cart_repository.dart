...
class CartRepository implements CartRepositoryInterface<OnlineCart> {
  ...
  Future<List<OnlineCartModel>?> _addToCartOnline(OnlineCart cart) async {
    ...
    Response response =
        await apiClient.postData('${AppConstants.addCartUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}', cart.toJson());
  ...
  Future<bool> _removeCartItemOnline(int cartId) async {
    Response response = await apiClient
        .deleteData('${AppConstants.removeItemCartUri}?cart_id=$cartId${!AuthHelper.isLoggedIn() ? '&guest_id=${AuthHelper.getGuestId()}' : ''}');
  ...
  Future<List<OnlineCartModel>?> _updateCartOnline(Map<String, dynamic> body) async {
    List<OnlineCartModel>? onlineCartList;
    Response response =
        await apiClient.postData('${AppConstants.updateCartUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}', body);
    if (response.statusCode == 200) {
      onlineCartList = [];
      response.body.forEach((cart) => onlineCartList!.add(OnlineCartModel.fromJson(cart)));
    }
  ...
  Future<bool> _updateCartQuantityOnline(int cartId, double price, int quantity) async {
    ...
    Response response =
        await apiClient.postData('${AppConstants.updateCartUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}', data);
    return (response.statusCode == 200);
  }
}
