...
import 'package:user_app_web/features/auth/widgets/min_max_time_picker_widget.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: GetBuilder<StoreRegistrationController>(builder: (storeRegController) {
          return Column(
            ...
            children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                MinMaxTimePickerWidget(
                  times: time,
                  onChanged: (int index) => storeRegController.minTimeChange(time[index]),
                  initialPosition: 10,
                ),
                Text(':', style: robotoBold),
                MinMaxTimePickerWidget(
                  times: time,
                  onChanged: (int index) => storeRegController.maxTimeChange(time[index]),
                  initialPosition: 10,
                ),
                MinMaxTimePickerWidget(
                  times: unit,
                  onChanged: (int index) => storeRegController.timeUnitChange(unit[index]),
                  initialPosition: 1,
                ),
              ]),
              Padding(
