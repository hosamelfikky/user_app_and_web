...
import 'package:user_app_web/features/checkout/domain/models/distance_model.dart';
...
import 'package:user_app_web/features/checkout/widgets/order_successfull_dialog.dart';
import 'package:user_app_web/features/checkout/widgets/partial_pay_dialog_widget.dart';
...
class CheckoutController extends GetxController implements GetxService {
  ...
  Future<void> placePrescriptionOrder(
    ...
    update();
    Response response =
        await checkoutServiceInterface.placePrescriptionOrder(storeId, distance, address, longitude, latitude, note, multiParts, dmTips, deliveryInstruction);
    _isLoading = false;
    if (response.statusCode == 200) {
      String? message = response.body['message'];
    ...
    if (isSuccess) {
      ...
      if (paymentMethodIndex == 2) {
        ...
        if (ResponsiveHelper.isDesktop(Get.context) && AuthHelper.isLoggedIn()) {
          ...
          Future.delayed(
              const Duration(seconds: 2), () => Get.dialog(Center(child: SizedBox(height: 350, width: 500, child: OrderSuccessfulDialog(orderID: orderID)))));
        } else {
