...
import 'package:user_app_web/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:user_app_web/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:user_app_web/features/checkout/widgets/bottom_section.dart';
import 'package:user_app_web/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
          ...
          ? GetBuilder<CheckoutController>(builder: (checkoutController) {
              ...
              return GetBuilder<CouponController>(builder: (couponController) {
                ...
                if (checkoutController.orderType != 'take_away' && checkoutController.store != null) {
                  _deliveryChargeForView = (checkoutController.orderType == 'delivery' ? checkoutController.store!.freeDelivery! : true)
                      ? 'free'.tr
                      : deliveryCharge != -1
                              ...
                              : Container(
                                  ...
                                  child: Column(
                                    children: [
                                      Padding(
                                        padding:
                                            const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge, vertical: Dimensions.paddingSizeExtraSmall),
  ...
  Widget _orderPlaceButton(CheckoutController checkoutController, bool todayClosed, bool tomorrowClosed, double orderAmount, double? deliveryCharge, double tax,
      double? discount, double total, double? maxCodOrderAmount, bool isPrescriptionRequired) {
    return Container(
      width: Dimensions.webMaxWidth,
                      ...
                      if (widget.storeId == null) {
                        ...
                        PlaceOrderBodyModel placeOrderBody = PlaceOrderBodyModel(
                          ...
                          unavailableItemNote: Get.find<CartController>().notAvailableIndex != -1
                              ...
                              : '',
                          deliveryInstruction:
                              checkoutController.selectedInstruction != -1 ? AppConstants.deliveryInstructionList[checkoutController.selectedInstruction] : '',
  ...
  double _calculateDiscount({required Store? store, required List<CartModel?>? cartList, required double price, required double addOns}) {
    ...
    if (store != null && cartList != null) {
      for (var cartModel in cartList) {
        double? dis =
            (store.discount != null && DateConverter.isAvailable(store.discount!.startTime, store.discount!.endTime)) && cartModel!.item!.flashSale != 1
  ...
  double _calculateDeliveryCharge(
    ...
    if (orderType == 'take_away' ||
        ...
        (Get.find<SplashController>().configModel!.freeDeliveryOver != null && orderAmount >= Get.find<SplashController>().configModel!.freeDeliveryOver!) ||
        Get.find<CouponController>().freeDelivery ||
        (AuthHelper.isGuestLoggedIn() && (Get.find<CheckoutController>().guestAddress == null && Get.find<CheckoutController>().orderType != 'take_away'))) {
      deliveryCharge = 0;
    }
